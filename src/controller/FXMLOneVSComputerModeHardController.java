 ...
 * @author mando
 */
public class FXMLOneVSComputerModeHardController implements Initializable {
    ...
    private void computerTurn(){
        ...
        for (int i = 0; i < 9; i++) {
            ...
            else if(btns[i].getText().equals("X")){
                board[i]=1;
            }
            else if(btns[i].getText().equals("O")){
                board[i]=2;
            }
        ...
        int[] res = new int[2];
        res = MINMAX.getBestMove(playBoard,9);
        int btnNum = res[0]*3+res[1];
        Button myBtn = null;
        if(!isFullGrid()){
            myBtn = btns[btnNum];
        }
